...
\tytulang{Framework for writing applications that improve office work}
...
% S³owa kluczowe:
\keywords{zr±b, wtyczka, REST, WWW, Django, AngularJS, OAuth}%fa
...
\begin{abstract}
W sprawnym dzia³aniu rozwijaj±cej siê firmy kluczow± rolê pe³ni± dane o jej przedsiêwziêciach, pracownikach i zasobach.
Nasza praca przedstawia zr±b eenet, który powsta³ w celu skutecznego zarz±dzania tymi informacjami.
\end{abstract}
...
Problemy zleceniodawcy wyniknê³y z szybkiego rozwoju firmy.
...
\item[\ref{r:podsumowanie}]  -- podsumowanie dotychczasowych osi±gniêæ zrêbu eenet oraz naszego zespo³u,
\item[\ref{r:kod}]           -- krótki opis podzia³u kodu aplikacji na katalogi i pliki.
\end{itemize}
...
\item wtyczki przyk³adowe -- wtyczki, które stworzono w ramach wykonania aplikacji eenet;
...
\item serwer produkcyjny -- komputer serwuj±cy stronê WWW, tu te¿ jest baza danych aplikacji;
\item serwer deweloperski (testowy) -- maszyna, która s³u¿y do testowania ka¿dej wersji aplikacji, zanim ta trafi na serwer produkcyjny;
...
\item RWD (ang. \textit{Responsive Web Design}) -- technika projektowania strony WWW tak, aby uk³ad tre¶ci dostosowywa³ siê automatycznie do szeroko¶ci okna przegl±darki b±d¼ szeroko¶ci ekranu urz±dzenia, na którym stronê siê przegl±da. Stronê wykonan± w tej technice nazywamy responsywn±;
...
mo¿na znacznie ograniczyæ koszty pisania poszczególnych funkcjonalno¶ci. ¦rodowisko zapewnia pomocnicze funkcjonalno¶ci wtyczkom (np. dostêp do bazy danych, logowanie u¿ytkowników, uprawnienia) -- wtyczki dziêki temu nie musz± same siê zajmowaæ tymi kwestiami -- takie rozumowanie jest zgodne z zasad± DRY.
...
Oczywi¶cie takie ujednolicenie wi±¿e siê z narzuceniem pewnych barier, lecz w naszej ocenie przy rozwi±zywaniu tego typu problemu ograniczenia pojawi± siê wcze¶niej czy pó¼niej, wiêc lepiej ustanowiæ je na samym pocz±tku.
...
\begin{itemize}
 \item udostêpnienie zaawansowanego systemu zarz±dzania u¿ytkownikami,
 \item stworzenie mechanizmu do inwentaryzacji dóbr i informacji,
 \item zautomatyzowanie procesu sk³adania pró¶b o urlopy w firmie.
\end{itemize}
...
\begin{itemize}
 ...
 \item \nameref{r:kafle}
\end{itemize}
Dwie kolejne napisane przez nas wtyczki rozwi±zuj± problemy z inwentaryzacj± i urlopami:
\begin{itemize}
 \item \nameref{r:urlopy}
 \item \nameref{r:zasoby}
\end{itemize}
...
Laboratorium EE postawi³o kilka wymagañ dotycz±cych tworzonej aplikacji. Po pierwsze rozwi±zanie musia³o byæ aplikacj± WWW, umo¿liwiaj±c± uruchomienie jej na prywatnym serwerze, aby mieæ pewno¶æ co do bezpieczeñstwa przechowywanych danych. Ponadto stworzona aplikacja musi mieæ mo¿liwo¶æ po³±czenia z pozosta³ymi wewn±trzfirmowymi produktami oraz spe³niaæ logikê biznesow± zaproponowan± przez Zamawiaj±cego. Ostatnim kryterium odno¶nie nowego produktu by³a jego cena.
...
Aplikacja z otwartym ¼ród³em pozwalaj±ca na przesy³anie plików, zarz±dzanie zasobami firmowymi oraz kierowanie projektami. Plusem tego rozwi±zania jest to ¿e darmowe oraz pozwala na dowolne tworzenie nowych aplikacji, natomiast minusem fakt, ¿e jest nie rozwijane, napisane w starej technologii oraz nie istnieje dokumentacja kodu.

\section{Podsumowanie rozwi±zañ konkurencyjnych}
Ka¿de ze znalezionych rozwi±zañ mia³o swoje dobre strony, jednak¿e ¿adne nie spe³nia³o wszystkich warunków postawionych przez Zamawiaj±cego.
...
\item realizuje API, z którymi komunikuje siê GUI.
\end{itemize}
...
Oprócz ustalenia serwera-bramy, nale¿y te¿ w jaki¶ sposób uruchomiæ serwer Django na serwerze produkcyjnym. W tym celu u¿yli¶my narzêdzia supervisord, które powoduje, ¿e nie trzeba pisaæ osobnego daemona do procesu Django.
...
Kluczow± rolê spe³nia wspomniany wcze¶niej zr±b Django. Oczekiwali¶my od back-endowego zrêbu nastêpuj±cych funkcjonalno¶ci:
\begin{itemize}
\item ³atwego dostêpu do bazy danych SQL,
\item obs³ugi sesji u¿ytkowników,
\item interfejsu do administracji danymi dla administratora,
\item mechanizmu autoryzacji OAuth zgodnego z Google OAuth,
\item mo¿liwo¶ci ³atwego stworzenia wielu koñcówek API dostêpnych pod ró¿nymi adresami URL,
\item narzucenia ograniczeñ na dostêp do API w zale¿no¶ci od przywilejów poszczególnych u¿ytkowników.
\end{itemize}
...
Koncepcja stron internetowych typu \textit{single-page application} pojawi³a siê w 2003 r. \cite{spa} i pierwsze realizacje zaczê³y pojawiaæ siê w podobnym czasie. Wówczas bardzo niszowe, obecnie staj± siê coraz bardziej popularne, g³ównie za spraw± implementacji w przegl±darkach History API i pojawieniu siê wielu zrêbów zapewniaj±cych ³atwe tworzenie \textit{single-page application}, takich jak AngularJS, Backbone czy Ember.
...
\item obiektowe podej¶cie do AJAXowej komunikacji z API dziêki serwisowi \verb+$resource+,
...
Jêzyk HTML ma wbudowane podstawowe znaczniki, które spe³niaj± jasno okre¶lon± rolê w interfejsie (np. \verb+button+, \verb+select+), jednak¿e jest du¿a dowolno¶æ w kwestii wizualnego zaplanowania interfejsu. Zamiast wymy¶laæ strukturê stron HTML ca³kowicie od podstaw i pisaæ w³asne style CSS upiêkszaj±ce domy¶lne formatowanie, skorzystali¶my z biblioteki Bootstrap wykonanej przez firmê Twitter. Dziêki niej stworzyli¶my tak± strukturê HTML strony, któr± mo¿na ³atwo modyfikowaæ.

Dodatkowo, style z Bootstrapa s± responsywne -- zatem nie musieli¶my siê przejmowaæ tym, ¿e trzeba napisaæ osobny kod HTML i osobne style dla u¿ytkowników smartfonów i tabletów.
...
Administrator nie ma ¿adnych dodatkowych uprawnieñ.
...
\subsection{Opis}
Wtyczka Urlopy pozwala na sk³adanie wniosków o przyznanie urlopu oraz zarz±dzanie nimi. Z ka¿dym u¿ytkownikiem zwi±zana jest liczba przys³uguj±cych mu wolnych dni do wykorzystania na urlopy.
W odniesieniu do tej wtyczki administratorem jest osoba z uprawnieniem \verb+admin+ we wtyczce Urlopy.
Z ka¿dym wnioskiem zwi±zana jest liczba dni roboczych w trakcie tego urlopu, która zostaje odjêta od liczby pozosta³ych dni wolnych danego u¿ytkownika.
Wtyczka Urlopy pozwala tak¿e na zmianê (przez uprawnione osoby) liczby dni wolnych, przys³uguj±cych dowolnemu u¿ytkownikowi.
...
\begin{enumerate}
\item
	Urlopobiorca klika przycisk ,,Z³ó¿ pro¶bê'' i wype³nia potrzebne dane:
	\begin{itemize}
...
Sama wtyczka Kafle s³u¿y do tego, ¿eby oprócz kafli zwi±zanych z wtyczkami móc tworzyæ kafle niestandardowe, które mo¿na tworzyæ bez modyfikacji kodu zrêbu. Przy pomocy wtyczki Kafle mo¿na takie kafle tworzyæ, ustawiaæ im kolor t³a (w przestrzeni RGB), tekst, obrazek i URL otwierany po ich klikniêciu.
...
W wielu przypadkach implementacja funkcjonalno¶ci eenetu sprowadza³a siê do postêpowania zgodnie z zasadami konkretnego zrêbu czy biblioteki. Przyk³adowo, gdy chcieli¶my stworzyæ w aplikacji kafle z hiper³±czami na stronie g³ównej, nale¿a³o stworzyæ:
\begin{itemize}
...
W celu zapamiêtania historii zmian w bazie danych zosta³o zaimplementowane wersjonowanie. Zosta³ do tego wykorzystany dodatek do Django o nazwie \verb+django-reversion+. Zapamiêtuje on wszystkie dokonane zmiany instancji djangowych modeli (np. u¿ytkownika, zasobu, wniosku o urlop) oraz pozwala na ³atwe przywracanie poprzednich wersji.
...
Koncepcje wszystkich czê¶ci projektu zosta³y stworzone przez ogó³ zespo³u podczas cotygodniowych spotkañ. Kod napisany przez ka¿dego z cz³onków zespo³u by³ poddawany rewizji przez resztê zespo³u, co pozwala³o na wy³apywanie ewentualnych b³êdów oraz wprowadzanie poprawek.

Poni¿ej znajduj± siê listy elementów projektu, które by³y tworzone przez poszczególnych cz³onków zespo³u. \vspace{1cm}
\\ 
Patryk Hes
\begin{itemize}
\item wtyczka Zasoby
...
Pawe³ Kamiñski
\begin{itemize}
\item wtyczka U¿ytkownicy
\end{itemize}
Borys Pop³awski
\begin{itemize}
\item wtyczka Urlopy
\item zbiór wspólnych narzêdzi front-endu u¿ywanych przez ró¿ne wtyczki
\end{itemize}
Robert Tomkowski
\begin{itemize}
\item wtyczka Kafle
\item wtyczka Powiadomienia
\item zbiór wspólnych narzêdzi back-endu u¿ywanych przez ró¿ne wtyczki
\item strona g³ówna
\end{itemize}
...
Kod ¼ród³owy zasadniczej czê¶ci zrêbu znajduje siê w katalogu \verb+eenet+.
Kody ¼ród³owe poszczególnych wtyczek znajduj± siê w katalogach:
\begin{itemize}
\item{U¿ytkownicy} -- \verb+custom_user+
\item{Zasoby} -- \verb+resources+
\item{Urlopy} -- \verb+urlopownik+
\item{Powiadomienia} -- \verb+notifications+
\item{Kafle} -- \verb+tileswithlinks+
\end{itemize}

W pliku \verb+README.md+ znajduje siê instrukcja instalowania eenetu -- zarówno wersji deweloperskiej, jak i produkcyjnej. Procedura instalacji zosta³a przewidziana na system operacyjny Debian ,,wheezy'' (7.0) z pow³ok± bash.
...
\bibitem{jquery} Dokumentacja jQuery -- \url{http://api.jquery.com/}
