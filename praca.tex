...
Praca sk³ada siê z~X rozdzia³ów i~dodatków.

W~rozdziale~\ref{r:pojecia} przypomniano podstawowe pojêcia u¿ywane w dokumencie.
Ogólny opis problemu wraz z prób± rozwi±zania go zawarto w~rozdziale~\ref{r:opis}.
Analizê konkurencyjnych rozwi±zañ zestawiono w~rozdziale~\ref{r:konkurencja}
%Dotychczasowe próby implementacji blablizatora
...
Okazuje siê, ¿e przy zarzadzaniu firm± do wielu problemów nie wystarczaj± arkusze kalkulacyjne, dokumenty tekstowe i komunikatory -- potrzeba oprogramowania, które nada zgromadzonym danym sens (kontekst).

Jako ¿e nie nikt nie by³ i nigdy nie bêdzie, w stanie zdefiniowaæ ostatecznych wymagañ dla problemu zarz±dzania firm±, zdecydowali¶my ¿e najlepszym rozwi±zaniem bedzie stworzenie zrêbu.
...
Oczywi¶cie takie ujednolicenie wi±¿e siê z narzuceniem pewnych barier, lecz w naszej ocenie przy rozwi±zywaniu tego typu problemu ograniczenia pojawia siê wcze¶niej czy pó¼niej, wiêc lepiej ustanowiæ je na samym poczatku.
...
W pocz±tkowej fazie negocjacji z~klientem wyniknê³y nastepujace wymagania dotycz±ce gotowej aplikacji przyk³adowej, nale¿y co nastepuje:
\begin{itemize}
...
\begin{itemize}
 \item U¿ytkownicy \ref{r:uzytkownicy}
 \item Powiadomienia \ref{r:powiadomienia}
 \item Kafle \ref{r:kafle}
\end{itemize}
Dwie kolejne rozwi±zuj± problemy z inwentaryzacj± i urlopami:
\begin{itemize}
 \item Urlopy \ref{r:urlopy}
 \item Zasoby \ref{r:zasoby}
\end{itemize}

Ca³e oprogramowanie powstawa³o przy ¶cis³ej wspó³pracy z klientem, zarówno przy ustalaniu specyfikacji jak i od strony merytorycznej podczas tworzenia eenetu.

Do koñcowej implementacji projektu eenet zosta³a dodana dokumentacja stworzona za pomoc± narzêdzi \textit{doxygen} i \textit{docular},
ponadto zosta³a utworzony podrêcznik u¿ytkownika oraz opis instalacji i konfiguracji aplikacji.
...
\section{AppFlower}
Open sourcowy framework do tworzenia ma³ych aplikacji, nie wymagaj±cy programowania, pozwalaj±cy na budowanie aplikacji z klocków. Plusem tego rozwi±zania jest to ¿e jest darmowy, natomiast minusem fakt, ¿e ³atwo¶æ tworzenia aplikacji powoduje zbyt ograniczon± funkcjonalno¶æ, jak na wiêksze potrzeby ma³ych i ¶rednich przedsiêbiorstw.
\section{OpenBiz AppBuilder}
P³atny framework do tworzenia ma³ych aplikacji, pozwalaj±cy na handlowanie nimi. Mocn± stron± tego rozwi±zania jest do¶æ du¿a dowolno¶æ w tworzeniu nowych aplikacji, natomiast s³absz± -- jej do¶æ wysoka cena oraz brak dobrej dokumentacji do ¶rodowiska tworzenia nowych wtyczek.
...
W odniesieniu do tej wtyczki administratorem jest osoba z uprawnieniem \verb+admin+ we wtyczce Urlopy, a koordynatorem -- menad¿er grupy(dowolnej), do której nale¿y u¿ytkownik. 
\\

Przyznawanie urlopów odbywa siê w kilku krokach:
\begin{enumerate}
\item U¿ytkownik sk³ada wniosek o urlop.
\item Wniosek zostaje zaakceptowany, odrzucony, albo odes³any do poprawki przez dowolnego koordynatora.
\item Je¶li wniosek zosta³ zaakceptowany przez koordynatora, to dowolny administrator mo¿e podj±æ akcje analogicznie jak w punkcie 2.
\end{enumerate}

Wniosek uznaje siê za zaakceptowany je¶li zostanie zaakceptowany przez administratora. Ponadto z ka¿dym wnioskiem zwi±zana jest liczba dni roboczych w trakcie tego urlopu, która zostaje odjêta od ilo¶ci pozosta³ych dni wolnych danego u¿ytkownika.

Dodatkowo wtyczka Urlopy pozwala na zmianê(przez uprawnione osoby) ilo¶ci dni wolnych, przys³uguj±cych dowolnemu u¿ytkownikowi.
...
Administrator mo¿e akceptowaæ, odrzucaæ lub cofaæ do poprawki wniski ju¿ zaakceptowane przez koordynatora. Ponadto mo¿e zmieniaæ liczbê dni wolnych przys³uguj±cych u¿ytkownikom.
...
\subsection{Opis}
G³ównym zadaniem wtyczki Zasoby jest zarz±dzanie oraz inwenteryzacja mienia fizycznego oraz cyfrowego firmy. Wtyczka umo¿liwa zarz±dzanie szeregiem predefiniowanych rodzajów dóbr, takich jak:
\begin{itemize}
\item biura,
\item monitory,
\item klucze ssh,
...
Ka¿dy rodzaj zasobu ma oddzielne uprawnienia dostêpu, np: ka¿dy mo¿e stworzyæ swój klucz ssh, lecz tylko administrator mo¿e tworzyæ nowe biura.

\subsection{Uprawnienia}
Ka¿dy u¿ytkownik mo¿e zarz±daæ zasobami, których jest w³a¶cicielem oraz przegl±daæ ogólno dostêpne zasoby.

Administratorzy mog± zarz±daæ wszystkimi zasobami.
...
\subsection{Opis}
Strona g³ówna aplikacji eenet zawiera w sobie szereg kafli, które spe³niaj± funkcjê obrazkowego menu. Ka¿dy kafel jest odnosnikiem do jakiej¶ podstrony lub strony zewnêtrznej. Wtyczka kafle powsta³a, by móc w ³atwy sposób tworzyæ odno¶niki do stron zewnêtrznych. Wtyczka ta pozwala na przegl±danie, edycjê i usuwanie ju¿ istniej±cych kafli oraz tworzenie nowych. Aby stworzyæ nowy kafel nale¿y podaæ:
\begin{itemize}
\item url(link), na który ma przekierowywaæ kafel;
\item kolor t³a w formacie RGB;
\item utl obrazka(opcjonalnie), który pojawi siê w kaflu.
\end{itemize}

\subsection{Uprawnienia}
Dostêp do wtyczni Kafle maj± tylko administratorzy.
...
Problemem okaza³o siê stworzenie systemu uprawnieñ, który pasowa³by do logiki aplikacji biznesowej potencjalnie ka¿dej wtyczki dzia³aj±cej w eenecie. Najbardziej rozwiniêtymi wtyczkami, jak± mieli¶my okazjê implementowaæ przy okazji implementowania samego zrêbu eenet by³y Urlopy\ref{r:urlopy} i Zasoby\ref{r:zasoby}.
...
%FIXME do wywalenia bo pokrywa sie z oraca wlozana w projekt(chyba zeby tam wypisac co zostalo zrobione a tu kto co konkretnie zrobil, do uzgodnienia)
